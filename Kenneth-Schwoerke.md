---
layout: resume # don't touch
categories: [ hire, graduation, austincodingacademy ] # don't touch

fullname: Kenneth Schwoerke
jobtitle: Student & Freelance Web Developer

location: Austin, TX
blurb: "Changing the world with Art and Code"

# first item must be C# or .NET or C#/.NET
# the first 3 will display on the card display, but list as many as you want, they will be visible on your hire page
skills:
  - JavaScript
  - React.js
  - CSS
  - Web Accessibility
  - HTML
  - MongoDB
  - SQL
  - Heroku
  - Git/GitHub
  - WebPack
  - REST APIs

github: https://github.com/Kjschwoerke
linkedin: https://www.linkedin.com/in/kenneth-schwoerke/

email: # kjschwoerke@yahoo.com
phone: # (361) 563-8574

# Make sure to edit these to be accurate for your work! The more relevant the better if the role was technical, don't feel like you need to put every job you've had.

workexperience:
  - company: New Critter Studios
    dates: October 2015 - Present
    title: Artist/Owner
    location: Central Texas

  - company: The University of Texas at San Antonio
    dates: March 2016 - August 2016
    title: Academic Advisor III
    location: San Antonio, TX

  - company: The University of Texas at San Antonio
    dates: February 2015 - March 2016
    title: Academic Advisor II
    location: San Antonio, TX

  - company: The University of Texas at San Antonio
    dates: June 2012 - February 2015
    title: Academic Advisor I
    location: San Antonio, TX



education:
  - name: Austin Coding Acaademy
    dates: June 2019 - May 2020
    degree: Web Development

  - name: Rackspace Open Cloud Academy
    dates: August 2016 - October 2016
    degree: Network Operations (CCNA - Expired)
    
  - name: Texas A&M University - Corpus Christi
    dates: Jan 2007 - December 2009
    degree: Master of Science: Counseling

  - name: Sam Houston State University
    dates: August 2003 - May 2006
    degree: Bachelor of Fine Arts: Art - Advertising and Graphic Design

  - name: The Art Institute of Houston
    dates: July 2001 - May 2003
    degree: Associate of Applied Science: Animation Art and Design

projects:
  - name: kjschwoerke.com
    desc: Personal Web Portfolio Site
    url:  https://kjschwoerke.com/
    
  - name: ArtFrog Art Academy
    desc: 2nd Place winning web-site AIR (Accessibility Internet Rally) 2019
    url:  http://artfrog.org/

  - name: New Critter Studios
    desc: Personal Artist Portfolio Site
    url:  https://agile-citadel-77839.herokuapp.com/

---